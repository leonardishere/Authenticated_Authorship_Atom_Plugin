...
import AuthenticatedAuthorshipStoreKeyView from './authenticated-authorship-storekey-view';
import AuthenticatedAuthorshipView3 from './authenticated-authorship-view3';
...
//  CryptoJS: require('crypto-js'),
  ...
  activate(){
    ...
    this.modalView4.getUsernameElement().onkeydown = function(event){
      if(event.key === "Tab"){
        if(event.shiftKey){
          ...
          self.modalView4.getPasswordPrivateElement().focus();
        }else{
    ...
    this.modalView4.getPasswordPrivateElement().onkeydown = function(event){
      if(event.key === "Tab"){
        if(event.shiftKey){
          self.modalView4.getPrivateKeyElement().focus();
        }else{
          self.modalView4.getUsernameElement().focus();
          //self.modalView4.getPrivateKeyStoreElement().focus();
        }
      }else if(event.key === "Enter"){
        signToTextHandler4();
      }else if(event.key === "Escape"){
    ...
    return function(){
      ...
      self.kbLogin.login(
        ...
        if(err) { console.log(err); }
          ...
          var uint8a = int32a_to_uint8a(step1.words);
  ...
  handleSignToHtmlWrapper2(self){
    return function(){
      ...
      .then((message) => {
        ...
        if(!message) message = "Signed article successfully!"
  ...
  handleSignToTextWrapper4(self){
    return function(){
      ...
      var hasAll = true;
      if(!username){
        atom.notifications.addError("Please add your username.");
        hasAll = false;
      }
      if(!password){
        atom.notifications.addError("Please add your keybase password.");
        hasAll = false;
      }
      if(!privateKeyTemp){
        atom.notifications.addError("Please upload your private key file.");
        hasAll = false;
      }
      if(!passwordPrivate){
        atom.notifications.addError("Please add your file password.");
        hasAll = false;
      }
      if(!hasAll) return;
  ...
  createPriKeyManager(obj) {
    ...
    return new Promise((resolve, reject) => {
      this.kbpgp.KeyManager.import_from_armored_pgp({armored: key}, (err, keyManager) => {
        if(err) {
          reject("Error creating private key manager.");
          return;
        }
  ...
  signToText(obj){
    ...
    return new Promise((resolve, reject) => {
      var signedArticle = "Use the Authenticated Authorship extension for Google Chrome to view this signature and validate my identity!\n"
      ...
      //console.log("signToText() resolved");
      return;
    });
  },
    ...
    return new Promise((resolve, reject) => {
      ...
      <script>
        ...
        for(var i = 0; i < articles.length; ++i){
          ...
          console.log("text: ", article.innerText);
          console.log("data: ", article.dataset);
        }
      </script>`;
      //signedArticle += script;
      editor.setText(signedArticle);
      resolve("Signed Article!");
    });
  },
  ...
  verify(){
    ...
    this.https.get(getAuthorInfoUrl, res => {
      ...
      res.on("end", () => {
        //body = public key
        if(body === "SELF-SIGNED PUBLIC KEY NOT FOUND" || body === "404 - sorry, not found."){
